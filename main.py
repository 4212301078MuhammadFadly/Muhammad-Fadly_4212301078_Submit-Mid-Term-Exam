...
print("TITLE : ATS MACHINE VISION")
print("Name  : Muhammad Fadly")
print("NIM   : 4212301078")
print("Class : MECHATRONICS 5C MORNING\n")

DATA_DIR = 'D:\\Muhammad Fadly\\SEMESTER 5\\Machine Vision\\ATS_4212301078_Muhammad Fadly\\data'
FILE_PATH = os.path.join(DATA_DIR, 'emnist-letters-train.csv')

OUTPUT_DIR = 'D:\\Muhammad Fadly\\SEMESTER 5\\Machine Vision\\ATS_4212301078_Muhammad Fadly\\output'
...
HOG_PARAMS_FINAL = {'orientations': 9, 'ppc': (6, 6), 'cpb': (2, 2)} 
SVM_PARAMS_FINAL = {'C': 1.0, 'kernel': 'linear'} 
...
def plot_confusion_matrix(y_true, y_pred, kernel_name):
    ...
    filename = f'hasil confusion_matrix_hog_svm_Muhammad Fadly_4212301078{kernel_name}.png'
...
def evaluate_loocv(X_features, y_labels, C_param, kernel_param):
    ...
    StandardScaler(),
    SVC(C=C_param, kernel=kernel_param, gamma='scale', random_state=42, max_iter=500000, tol=1e-4)
    )
...
## 5. Evaluasi Cepat (5-Fold Cross Validation)
def evaluate_kfold(X_features, y_labels, C_param, kernel_param, k=5):
    """Melakukan evaluasi cepat menggunakan Stratified K-Fold Cross Validation."""
    write_log(f"\n[K-FOLD CV] Memulai evaluasi cepat dengan {k}-Fold (Kernel={kernel_param}, C={C_param})...")
    ...
    start_time = time.time()
    scores = cross_val_score(model_svm, X_features, y_labels, cv=cv, n_jobs=-1)
    end_time = time.time()

    mean_acc = np.mean(scores)
    std_acc = np.std(scores)

    write_log("\n--- Hasil Metrik Evaluasi K-Fold ---")
    write_log(f"Waktu komputasi total: {(end_time - start_time):.2f} detik")
    write_log(f"Akurasi rata-rata ({k}-Fold): {mean_acc * 100:.2f}% ± {std_acc * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(FILE_PATH):
        ...
        # --- FASE UJI CEPAT: K-FOLD CV (OPSIONAL) ---
        # write_log("\n---------- FASE UJI CEPAT: 5-FOLD CROSS VALIDATION ----------")
        # X_features_final = extract_hog_features(X, **HOG_PARAMS_FINAL)
        # from sklearn.preprocessing import StandardScaler
        # scaler = StandardScaler()
        # X_features_final = scaler.fit_transform(X_features_final)
        # _ = evaluate_kfold(X_features_final, y, 
        #            SVM_PARAMS_FINAL['C'], 
